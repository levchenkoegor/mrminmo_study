...
fig_dir_betas = group_analysis_dir / 'plots_bvalues_outter'
fig_dir_tvals = group_analysis_dir / 'plots_tvalues_outter'
fig_dir_betas.mkdir(parents=True, exist_ok=True)
fig_dir_tvals.mkdir(parents=True, exist_ok=True)

# Load extracted data for outside-brain
df = pd.read_csv(group_analysis_dir / 'df_b_t_values_outsidebrain.csv')

# Define plotting function (no hemisphere split)
def plot_histograms_outer(df, value_col, out_dir, label):
    for movement in df["movement"].unique():
        for roi in df["ROI"].unique():  # will only be 'outside_brain' in your case
            # Filter data
            data_minmo = df[
            ...
            plt.title(f"{label}: {movement} | ROI: {roi}")
            plt.xlabel(label)
            plt.ylabel("Count")
            plt.legend()
            plt.tight_layout()

            outname = f"histogram_outer_{label}_{movement}_{roi}.png".replace("/", "-")
            plt.savefig(out_dir / outname, dpi=300)
            plt.close()

# Run plotting for both beta values and t-values
plot_histograms_outer(df, 'beta_coef', fig_dir_betas, 'Beta Coefficient')
plot_histograms_outer(df, 't_value', fig_dir_tvals, 'T-Value')

print("All histograms saved for outside-the-brain beta coefficients and t-values.")
